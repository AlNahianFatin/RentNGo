...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($reward)) {
        $errors[] = "Please enter a reward scheme first.";
        header('location: Admin_LoyaltyProgramAdd_F.php?error=invalidRewardScheme');
    }
    
    if ($points === "" || !is_numeric($points) || $points < 0) {
        $errors[] = "Please enter valid reward points.";
        header('location: Admin_LoyaltyProgramAdd_F.php?error=invalidRewardPoint');
    }
    
    if (empty($type)) {
        $errors[] = "Please select amenities type first.";
        header('location: Admin_LoyaltyProgramAdd_F.php?error=invalidAmentityType');
    }

    if (empty($errors)) {
        // echo "Reward scheme added successfully.";
    } 
    else {
...
<body>
    ...
    <form action="Admin_LoyaltyProgramAdd_F.php" method="post" style="margin-top: 5vh;" onsubmit="return validateAddScheme(event)">
        <fieldset>
    ...
    <a href="Customer_LoyaltyProgram_F.html">
